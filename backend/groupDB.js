...
module.exports = {
    ...
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO groups (name, courseInstance, owner) 
                ...
                term = (SELECT term from term where active) AND code = ?), ?)`,
        [group_name, course, user],
        err => {
          if (err) {
            reject(err);
          }
  ...
  deleteGroup: function(group_name, course) {
    return new Promise((resolve, reject) => {
      ...
      db.run(
        `DELETE FROM groupUsers WHERE id IN (
  ...
  addUsertoGroup: function(username, group_name, course) {
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO groupUsers (groupid, username) VALUES 
            ( 
                (SELECT groups.id FROM groups 
                LEFT JOIN courseInstance on groups.courseInstance = courseInstance.id
                WHERE name = ?
