...
_db = new sqlite3.Database("test.db", err => {
    if (err) {
...
_db.exec(schema_query, function(err) {
if (err) {
  console.log(err.message);
}
});

_db.run(insert_query, ranks, function(err) {
if (err) {
  console.log(err.message);
}
});
...
// batch entry 10-20 at once!
    const {code, name} = entry;
    coursedb.addCourse(code, name).catch(err => {console.log(err)});
});
*/
